...
class Board:
    ...
    def draw_circle(sc, x, y, size):
        x = (x + .5) * size
        y = (y + .5) * size
        pygame.draw.circle(sc, 'red', (x, y), (size - 3) // 2, 3)

    @staticmethod
    def draw_cross(sc, x, y, size):
        x = x * size + 3
        y = y * size + 3
        pygame.draw.line(sc, 'blue', (x, y), (x + size - 3, y + size - 3), 3)
        pygame.draw.line(sc, 'blue', (x + size - 3, y - 3,), (x, y + size - 3), 3)
